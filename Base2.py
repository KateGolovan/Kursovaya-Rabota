...
class working_process:
    def __init__(self, game):
        self.game = game
        self.field_image = self.get_scaled_image(path='Image/field42_pgm.png', res=[WIN_SIZE] * 2)
        self.O_image = self.get_scaled_image(path='Image/nolik11_pgm.png', res=[CELL_SIZE] * 2)
        self.X_image = self.get_scaled_image(path='Image/krestik11_pgm.png', res=[CELL_SIZE] * 2)
    ...
    def draw_winner(self):
        if self.winner:
            ...
            label = self.font.render(f'Игрок "{self.winner}" выйграл!', True, 'white', 'black')
    ...
    def print_caption(self):
        pg.display.set_caption(f'Игрок "{"OX"[self.player]}" ходит!')
        if self.winner:
            pg.display.set_caption(f'Игрок "{self.winner}" выйграл! Нажмите "Пробел", чтобы начать заново. ')
        elif self.game_steps == 9:
            pg.display.set_caption(f'Ничья! Нажмите "Пробел", чтобы начать заново. ')
